 ...
 */
class Player21 implements PlayerActions
{
    public const
      /**
    ...
    public function hit(Card $cardForTesting = null)
    {
        ...
        if (
         strtolower($pulledCard->getTypeOfCard()) === 'ace'
         && $this->currentScore > Game21::BLACKJACK
      ) {
            $this->currentScore -= 10;
        }
    ...
    {
        if (strtolower($this->type) === 'player' && $this->currentScore >= Game21::BLACKJACK) {
            $this->stand();
        } else {
            Game21::refresh();
        }
